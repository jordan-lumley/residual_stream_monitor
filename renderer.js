...
var crdPivotFile = __dirname + "/consolidated_residual_details_for_pivot_table.csv";
...
consolidateFilesBtn.addEventListener('click', async (e) => {
    try {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            const csv = await CsvParser.parseCsv(fileElement.innerText, { skipRows: 1 });
            const pattern = /\_(.*?)\_/
            for (let x = 0; x < csv.data.length; x++) {
                ...
                csv.data[x].date = moment().subtract(1, 'months').startOf('month').format('MM/DD/YYYY')
            }
        ...
        var cardStepContentWrapper = document.getElementById('step1CardContentWrapper')
...
beginDbImportBtn.addEventListener('click', async (e) => {
    try {
        ...
        var cardStepContentWrapper = document.getElementById('step2CardContentWrapper')
        cardStepContentWrapper.innerHTML = '';
...
runNatesQueryBtn.addEventListener('click', async (e) => {
    try {
        ...
        var cardStepContentWrapper = document.getElementById('step3CardContentWrapper')
        cardStepContentWrapper.innerHTML = '';
...
buildResultsBtn.addEventListener('click', async () => {
    try {
        ...
        console.log(`amtAfterCostExpenseSum ${resultSum.amtAfterCostExpenseSum.sum}`)
        console.log(`amtAfterCostRevenueSum ${resultSum.amtAfterCostRevenueSum.sum}`)
        ...
        var cardStepContentWrapper = document.getElementById('step4CardContentWrapper')
        cardStepContentWrapper.innerHTML = '';
